...
The decision to create a top-level `dp/` workspace is correct. A production data platform should not
...
|---|---|---|
| Top-level boundary | Good | Keep `dp/` beside `ai/` and `backend/`. |
...
dp/
  ...
  src/enterprise_dp/      # validation and platform developer tooling
