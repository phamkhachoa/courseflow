...
`enterprise-dp catalog-export` creates a source-controlled metadata bundle that can later be pushed
...
Use `enterprise-dp openlineage-export` to convert a bundle plus run manifests into OpenLineage-style
runtime events for a lineage backend or async publisher.

Use `enterprise-dp catalog-publish-manifest` after catalog, lineage and semantic-view artifacts are
...
```bash
cd dp
PYTHONPATH=src python -m enterprise_dp.cli catalog-export \
  --root . \
...
PYTHONPATH=src python -m enterprise_dp.cli catalog-publish-manifest \
