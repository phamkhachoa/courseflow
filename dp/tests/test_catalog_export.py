...
from enterprise_dp.catalog import build_catalog_bundle, write_catalog_bundle
...
def test_catalog_bundle_exports_enterprise_metadata_and_static_lineage() -> None:
    ...
    assert gold["urn"] == "urn:enterprise-dp:data-product:gold.recsys_interactions"
    assert gold["contract_urn"] == "urn:enterprise-dp:data-product:gold.recsys_interactions:v1"
    ...
    assert topic["urn"] == "urn:enterprise-dp:topic:recommendation.tracking.v1"
    assert topic["contract_urn"] == "urn:enterprise-dp:topic:recommendation.tracking.v1:v1"
...
def test_catalog_bundle_includes_run_evidence_from_ingestion_and_gold_manifests(tmp_path: Path) -> None:
    ...
    assert [(edge["source"], edge["target"]) for edge in run_edges] == [
        (
            "urn:enterprise-dp:data-product:bronze.events_recommendation_tracking",
            "urn:enterprise-dp:data-product:silver.learner_activity",
        ),
        (
            "urn:enterprise-dp:data-product:silver.learner_activity",
            "urn:enterprise-dp:data-product:gold.recsys_interactions",
        ),
    ]
