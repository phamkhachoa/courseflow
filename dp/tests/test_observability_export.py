...
from enterprise_dp.observability import (
...
def test_observability_export_writes_metrics_and_summary_without_high_cardinality_labels(tmp_path: Path) -> None:
    ...
    metrics_path = tmp_path / "observability" / "enterprise-dp.prom"
    ...
    assert "enterprise_dp_release_gate_status" in metrics
    assert 'gate_id="P0-FRESHNESS"' in metrics
    assert "enterprise_dp_data_product_freshness_seconds" in metrics
    assert "enterprise_dp_ingestion_lag_seconds" in metrics
    assert "enterprise_dp_cost_attribution_rows_total" in metrics
    ...
    assert summary["artifact_type"] == "enterprise_dp_observability_snapshot.v1"
...
def test_observability_export_emits_failed_gate_zero(tmp_path: Path) -> None:
    ...
    gate_metrics = [
        ...
        if metric["name"] == "enterprise_dp_release_gate_status"
