...
from enterprise_dp.change_requests import load_reference_sets, maker_checker_separated
from enterprise_dp.contracts import DATA_PRODUCT_NAME, PRODUCT_CODE, ValidationResult, load_yaml, require_int, require_mapping, require_string
...
def report_payload(
    ...
    *,
    ...
    return {
        ...
        "environment": environment,
        "request_id": request_id,
        "mode": backfill_readiness_mode(environment, passed=passed, evidence=evidence_refs),
        ...
        "summary": backfill_readiness_summary(request, checks, evidence_refs),
        "checks": checks,
        "failures": [
...
def backfill_readiness_mode(environment: str, *, passed: bool, evidence: dict[str, Any]) -> str:
    if (
        environment in PRODUCTION_LIKE_ENVIRONMENTS
        and passed
        and all(evidence_ref_attached(evidence.get(key)) for key in required_backfill_evidence_keys())
    ):
...
def backfill_readiness_summary(
    request: dict[str, Any] | None,
    checks: list[dict[str, Any]],
    evidence: dict[str, Any],
) -> dict[str, Any]:
    failed_checks = [item for item in checks if item.get("passed") is not True]
    request_controls = request.get("controls") if isinstance(request, dict) and isinstance(request.get("controls"), dict) else {}
    approvals = request.get("approvals") if isinstance(request, dict) and isinstance(request.get("approvals"), list) else []
    approved_roles = sorted(
        {
            str(approval.get("role"))
            for approval in approvals
            if isinstance(approval, dict) and approval.get("decision") == "approved" and approval.get("role")
        }
    )
    return {
        ...
        "attached_evidence_count": sum(1 for value in evidence.values() if evidence_ref_attached(value)),
        "request_status": request.get("status") if isinstance(request, dict) else None,
        "risk_level": request.get("riskLevel") if isinstance(request, dict) else None,
        "operation_type": request.get("operationType") if isinstance(request, dict) else None,
        "approved_role_count": len(approved_roles),
        "approved_roles": approved_roles,
        "maker_checker_required": request_controls.get("makerCheckerRequired"),
        "rollback_required": request_controls.get("rollbackRequired"),
        "impact_assessment_required": request_controls.get("impactAssessmentRequired"),
        "communication_required": request_controls.get("communicationRequired"),
    }


def required_backfill_evidence_keys() -> tuple[str, ...]:
    return (
        "backfill_plan",
        "active_pointer",
        "dry_run",
...
def evidence_ref_attached(value: object) -> bool:
    return (
        isinstance(value, dict)
        and value.get("local") is True
        and is_hash(value.get("hash"))
        and non_empty(value.get("uri"))
    )
