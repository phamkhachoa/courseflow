...
from enterprise_dp.ingestion import read_jsonl, write_jsonl
from enterprise_dp.source_registry import find_source_entry
...
def token_hash(value: Any) -> str | None:
    if value in (None, ""):
        return None
    return hashlib.sha256(f"enterprise-dp-token:{value}".encode("utf-8")).hexdigest()
