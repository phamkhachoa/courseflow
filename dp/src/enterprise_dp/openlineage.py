...
DEFAULT_NAMESPACE = "enterprise-dp://local"
DEFAULT_PRODUCER = "https://enterprise-dp.local/openlineage-export"
ENTERPRISE_DP_FACET_SCHEMA_BASE = "https://enterprise-dp.local/schemas/openlineage"
    ...
    *,
    ...
    return {
        ...
        "run": {
            ...
            "facets": {
                "enterpriseDp_run": enterprise_run_facet(run, producer),
            },
        },
        "job": {
            "namespace": namespace,
            ...
            "facets": {
                "enterpriseDp_job": enterprise_job_facet(run, producer),
            },
        },
        "inputs": [
...
def dataset_entry(
    ...
    *,
    ...
    facets: dict[str, Any] = {
        "enterpriseDp_dataset": enterprise_dataset_facet(name, kind, metadata or {}, producer),
    }
...
def enterprise_run_facet(run: dict[str, Any], producer: str) -> dict[str, Any]:
    return {
        "_producer": producer,
        "_schemaURL": f"{ENTERPRISE_DP_FACET_SCHEMA_BASE}/EnterpriseDpRunFacet.v1.json",
        "runUrn": run.get("urn"),
...
def enterprise_job_facet(run: dict[str, Any], producer: str) -> dict[str, Any]:
    return {
        "_producer": producer,
        "_schemaURL": f"{ENTERPRISE_DP_FACET_SCHEMA_BASE}/EnterpriseDpJobFacet.v1.json",
...
def enterprise_dataset_facet(name: str, kind: str, metadata: dict[str, Any], producer: str) -> dict[str, Any]:
    ...
    return {
        "_producer": producer,
        "_schemaURL": f"{ENTERPRISE_DP_FACET_SCHEMA_BASE}/EnterpriseDpDatasetFacet.v1.json",
