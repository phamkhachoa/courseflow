...
Operations governance also needs a registry-wide queue, not only release-scoped evidence. The
`enterprise-dp access-grant-ops-report` command scans every grant and produces a decision board for:

- P0 page-now issues: expired active grants, orphaned data products, missing approvals, missing
  required evidence, maker-checker conflicts, missing org scope and PII export exceptions.
- Review operations: overdue access reviews and review cadence drift from the consumer contract.
- Lifecycle operations: grants expiring within the warning window, overlong grants and redistribution
  exceptions that need explicit governance.
...
```bash
enterprise-dp change-control-check \
  --root dp \
