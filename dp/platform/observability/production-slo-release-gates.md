...
| `quality_slo_release_gates_ops_passed` | Aggregated quality/SLO operations artifact consumed by Control Tower. | `quality_slo_release_gates_ops_report.v1` has a valid artifact type, matching environment, correct readiness state, passing release quality/freshness gates, passing runtime quality evidence, green alert evidence and no product-level quality/SLO issues. | Block Control Tower production signoff and open an incident/SLO queue item. |
...
|---|---|---|
| `enterprise_dp_ingestion_lag_seconds` | `environment`, `product`, `topic`, `source_service`, `partition` | Event publish time to Bronze landing time. Emit p50, p95 and p99 or raw observations that can derive them. |
| `enterprise_dp_source_backlog_records` | `environment`, `product`, `topic`, `partition` | Consumer backlog or unprocessed source records. |
| `enterprise_dp_partition_stalled` | `environment`, `product`, `topic`, `partition` | Boolean gauge, `1` when the active source partition has not advanced inside its SLO. |
| `enterprise_dp_data_product_freshness_seconds` | `environment`, `product`, `data_product`, `layer` | Latest complete materialization age. |
| `enterprise_dp_quality_check_status` | `environment`, `product`, `data_product`, `check`, `severity` | Gauge, `1` pass and `0` fail. Required checks use `severity=blocking`. |
| `enterprise_dp_quality_failed_rows` | `environment`, `product`, `data_product`, `check` | Failed row count for the evaluated window or snapshot. |
| `enterprise_dp_quarantine_rows` | `environment`, `product`, `data_product`, `reason` | Quarantined rows excluded from publish. |
| `enterprise_dp_contract_compatibility_status` | `environment`, `product`, `contract`, `compatibility_mode` | Gauge, `1` compatible and `0` incompatible. |
| `enterprise_dp_gold_publish_evidence_status` | `environment`, `product`, `data_product` | Gauge, `1` only when the active Gold snapshot has a complete evidence pack. |
| `enterprise_dp_release_gate_status` | `environment`, `product`, `domain`, `use_case`, `gate_id` | Gauge, `1` when the release evidence gate passed and `0` otherwise. |
| `enterprise_dp_cost_attribution_rows_total` | `environment`, `product`, `domain`, `pipeline` | Row-count based proxy for product/domain/pipeline cost attribution until runtime cost collectors exist. |
...
| `validator_output_uri` | All P0 releases | Stored output from `cd dp && make check`. |
...
The aggregate quality/SLO operations artifact is `quality_slo_release_gates_ops_report.v1`; its
schema and CLI are defined in
`platform/observability/quality-slo-release-gates-ops.md`. Production-like reports must attach local
runtime quality evidence and SLO alert evidence, even when the source systems are external.
...
```json
{
  ...
  "subject_uri": "s3://dp-evidence/schema-registry/release-123.json",
  "subject_hash": "sha256:...",
  "environment": "prod",
...
keys. The same verifier is available as `enterprise-dp attestation-verify` for pre-release
...
1. Validate contracts and project structure with `cd dp && make check`.
...
6. Write the aggregate Quality/SLO operations report with
   `enterprise-dp quality-slo-release-gates-ops-report` and attach it to Control Tower.
7. Create a release promotion manifest with `enterprise-dp release-promote`; promotion is blocked
...
8. Create a release activation manifest with `enterprise-dp release-activate`; activation updates the
...
9. Publish or expose the target Gold data product only after `P0-LAKEHOUSE-SNAPSHOT-EVIDENCE`,
   `P0-GOLD-EVIDENCE`, promotion and activation are green.
10. Monitor the same gates for at least one Gold freshness interval after release.
...
- P0 gate breaches follow the runbook in `dp/ops-runbooks/data-platform-p0-gates.md`.
