...
- Deduplicate using the idempotency standard from
  `dp/platform/ingestion/source-offset-hash-idempotency.md`.
...
- `backfill_readiness_report.v1` from `enterprise-dp backfill-readiness-check` before
...
- Evidence includes `enterprise-dp snapshot-evidence-record` output for every Silver/Gold release
