...
- `dp/contracts/topics/*` and Bronze data product contracts for CourseFlow P0 events.
...
- Topic versioning gap: backend outbox topics are raw `event_type` values, while
  DP contracts use versioned topic names. The platform needs a compatibility
  bridge, producer change, or ingestion-side alias map.
- Envelope gap: many backend events are raw JSON payloads, not the DP shared
...
- Idempotency: backend consumers use `processed_events` tables or natural keys.
  ...
  event id otherwise.
- Contract status: current DP topic and data product contracts are `DRAFT`.
...
- Add DP contracts for selected P1 assessment, incentive, loyalty, and marketing
  funnel events.
