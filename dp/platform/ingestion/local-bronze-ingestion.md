...
`enterprise-dp ingest-bronze` is the local P0 preflight for contract-checked JSONL ingestion. It is
...
`enterprise-dp source-bridge-normalize` before Bronze ingestion. The bridge is a product-agnostic
...
```bash
cd dp
PYTHONPATH=src python -m enterprise_dp.cli source-bridge-normalize \
  --root . \
