...
variable "region" {
  type        = string
  description = "Primary cloud region for the managed production data-platform runtime."
}

variable "secondary_region" {
  type        = string
  description = "Secondary cloud region used by DR, replication and restore drills."
}
...
variable "environment" {
  ...
  validation {
    ...
    error_message = "This root module is bound to the production runtime profile."
  }
}
...
variable "cost_center" {
  type        = string
  description = "FinOps cost allocation tag for production runtime resources."
}
...
variable "change_request_id" {
  type        = string
  description = "Approved production change request that authorizes this apply."

  validation {
    condition     = length(trimspace(var.change_request_id)) > 0
    error_message = "Production applies require an approved change request id."
  }
}

variable "production_signoff_approved" {
  type        = bool
  description = "Explicit production signoff switch controlled outside this module."
  default     = false

  validation {
    condition     = var.production_signoff_approved
    error_message = "Production applies require production_signoff_approved=true."
  }
}
...
locals {
  common_tags = {
    ...
    ChangeRequestId = var.change_request_id
    ManagedBy       = "opentofu"
  }
...
module "event_backbone" {
  ...
  runtime = merge(local.runtime_defaults, {
    ...
    dr_tier           = "tier1"
    evidence_required = ["broker_health", "topic_acl", "consumer_lag", "dr_exercise"]
  })
}
...
module "cdc_connect" {
  ...
  runtime = merge(local.runtime_defaults, {
    ...
    backup_required   = true
    dr_tier           = "tier1"
    evidence_required = ["connector_status", "source_offset", "schema_history", "backup_restore"]
  })
}
...
module "schema_registry" {
  ...
  runtime = merge(local.runtime_defaults, {
    ...
    dr_tier                 = "tier1"
    evidence_required       = ["subject_export", "compatibility_report", "registry_backup", "restore_test"]
  })
}
...
module "object_storage" {
  ...
  runtime = merge(local.runtime_defaults, {
    ...
    backup_required   = true
    dr_tier           = "tier1"
    evidence_required = ["bucket_policy", "lifecycle_policy", "encryption_report", "cross_region_replication"]
  })
}
...
module "lakehouse_catalog" {
  ...
  runtime = merge(local.runtime_defaults, {
    ...
    backup_required    = true
    dr_tier            = "tier1"
    evidence_required  = ["catalog_snapshot", "table_property_export", "snapshot_manifest", "pitr_restore"]
  })
}
...
module "orchestration" {
  ...
  runtime = merge(local.runtime_defaults, {
    ...
    alert_rules           = true
    dr_tier               = "tier1"
    evidence_required     = ["asset_materialization", "retry_history", "run_storage_backup", "schedule_tick_history"]
  })
}
...
module "observability" {
  ...
  runtime = merge(local.runtime_defaults, {
    ...
    finops_budget     = true
    evidence_required = ["slo_dashboard", "alert_rules", "oncall_route", "metrics_retention", "budget_alert"]
  })
}

output "runtime_service_matrix" {
  description = "P0 runtime services declared by the production managed runtime root module."
  value = {
