...
from enterprise_df.environments import REQUIRED_P0_SERVICES
...
def test_staging_runtime_readiness_passes_with_complete_machine_evidence(tmp_path: Path) -> None:
    evidence = write_runtime_evidence_set(tmp_path, environment="staging", profile_id="staging-opentofu")
    report = build_runtime_readiness_report(
        ROOT,
        environment="staging",
...
def test_prod_runtime_readiness_passes_with_complete_machine_evidence(tmp_path: Path) -> None:
    evidence = write_runtime_evidence_set(tmp_path, environment="prod", profile_id="prod-opentofu", include_dr=True)
    report = build_runtime_readiness_report(
        ROOT,
        environment="prod",
        iac_plan_path=evidence["iac_plan"],
...
def test_runtime_readiness_rejects_environment_mismatched_evidence(tmp_path: Path) -> None:
    evidence = write_runtime_evidence_set(tmp_path, environment="prod", profile_id="prod-opentofu")
    report = build_runtime_readiness_report(
        ROOT,
        environment="staging",
        iac_plan_path=evidence["iac_plan"],
...
def test_runtime_evidence_pack_writer_creates_ci_pack_that_passes_staging_readiness(tmp_path: Path) -> None:
    ...
    assert pack.manifest["passed"] is True
    assert pack.manifest["production_signoff_allowed"] is True
...
def test_runtime_evidence_pack_cli_creates_prod_ci_pack_with_dr(tmp_path: Path) -> None:
    ...
    assert summary["production_signoff_allowed"] is True
    manifest = json.loads((output_dir / "runtime-evidence-pack-manifest.json").read_text(encoding="utf-8"))
