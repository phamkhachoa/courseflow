...
from enterprise_df.openlineage import (
...
def test_openlineage_export_builds_runtime_events_without_raw_payloads(tmp_path: Path) -> None:
    ...
    events = build_openlineage_events(
        bundle,
        namespace="enterprise-df://test",
        producer="https://enterprise-df.local/test-openlineage",
    )
    ...
    assert ingestion_event["inputs"][0]["namespace"] == "enterprise-df://test/topics"
    ...
    assert ingestion_event["run"]["facets"]["enterpriseDf_run"]["sourcePositions"]
...
def test_write_openlineage_events_and_cli_export(tmp_path: Path) -> None:
    ...
    result = write_openlineage_events(
        catalog_path,
        output_path,
        namespace="enterprise-df://local-test",
        producer="https://enterprise-df.local/test-openlineage",
    )
    records = [
        json.loads(line)
    ...
    assert records[0]["producer"] == "https://enterprise-df.local/test-openlineage"
    ...
    completed = subprocess.run(
        [
            ...
            str(cli_output),
            "--namespace",
            "enterprise-df://cli-test",
            "--producer",
            "https://enterprise-df.local/cli-openlineage",
        ],
        check=False,
        capture_output=True,
        text=True,
        ...
        [
            {
                ...
                "run": {"runId": ""},
                "job": {"namespace": "enterprise-df://test", "name": "bad-job"},
                "inputs": [{}],
                "outputs": [],
            }
        ]
