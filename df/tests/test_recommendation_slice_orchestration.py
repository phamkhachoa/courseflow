...
from enterprise_df.attestations import attestation_signing_payload
from enterprise_df.orchestration import run_recommendation_slice
...
from enterprise_df.release import build_recommendation_release_evidence
from enterprise_df.release_profiles import hash_release_profile_registry
from enterprise_df.schema_registry import write_schema_registry_report
from enterprise_df.snapshot_evidence import data_product_contract, write_snapshot_evidence_report
...
def test_recommendation_slice_requires_production_evidence_for_prod(tmp_path: Path) -> None:
    ...
    unverified = run_recommendation_slice(
        ...
        evaluation_time=EVALUATION_TIME,
        code_commit_sha="abc123",
        schema_registry_report_uri="s3://df-evidence/schema-registry/prod-unverified.json",
        schema_registry_report_hash="sha256:abc123",
        validator_output_uri="s3://df-evidence/validators/prod-unverified.json",
        access_policy_check_id="access-check-001",
        access_policy_report_uri="s3://df-evidence/access-policy/prod-unverified.json",
        access_policy_report_hash="sha256:def456",
        access_grant_evidence_uri="s3://df-evidence/access-grants/prod-unverified.json",
        access_grant_evidence_hash="sha256:grant789",
        retention_evidence_uri="s3://df-evidence/retention/prod-unverified.json",
        retention_evidence_hash="sha256:retention012",
        snapshot_evidence_uri="s3://df-evidence/snapshots/prod-unverified.json",
    ...
    complete_evidence = build_recommendation_release_evidence(
        ...
        validator_output_uri="s3://df-evidence/validators/prod-complete.json",
        access_policy_check_id="access-check-001",
...
def test_recommendation_slice_rejects_retention_attestation_bound_to_wrong_content(tmp_path: Path) -> None:
    ...
    result = run_recommendation_slice(
        ...
        schema_registry_report_uri=schema_attestation.as_posix(),
        schema_registry_report_hash=sha256_file(schema_attestation),
        validator_output_uri="s3://df-evidence/validators/prod-retention-attestation-wrong-content.json",
        access_policy_check_id="access-check-001",
        access_policy_report_uri=access_policy_attestation.as_posix(),
        access_policy_report_hash=sha256_file(access_policy_attestation),
        access_grant_evidence_uri=access_grant_attestation.as_posix(),
...
def test_recommendation_slice_builds_prod_retention_report_from_job_evidence_input(tmp_path: Path) -> None:
    ...
    result = run_recommendation_slice(
        ...
        schema_registry_report_uri=schema_attestation.as_posix(),
        schema_registry_report_hash=sha256_file(schema_attestation),
        validator_output_uri="s3://df-evidence/validators/prod-retention-input.json",
        access_policy_check_id="access-check-001",
        access_policy_report_uri=access_policy_attestation.as_posix(),
        access_policy_report_hash=sha256_file(access_policy_attestation),
        access_grant_evidence_uri=access_grant_attestation.as_posix(),
...
def test_recommendation_slice_rejects_prod_attestation_environment_mismatch(tmp_path: Path) -> None:
    ...
    result = run_recommendation_slice(
        ...
        schema_registry_report_uri=schema_attestation.as_posix(),
        schema_registry_report_hash=sha256_file(schema_attestation),
        validator_output_uri="s3://df-evidence/validators/prod-attestation-env-mismatch.json",
        access_policy_check_id="access-check-001",
        access_policy_report_uri=access_policy_attestation.as_posix(),
        access_policy_report_hash=sha256_file(access_policy_attestation),
        access_grant_evidence_uri=access_grant_attestation.as_posix(),
...
def write_external_attestation(
    ...
    *,
    ...
    payload = {
        ...
        "subject_uri": f"s3://df-evidence/{evidence_kind}/prod-complete.json",
...
def write_recommendation_snapshot_evidence(path: Path, *, release_id: str, ingestion: object, medallion: object) -> Path:
    ...
    ledger = write_offset_ledger_report(
        ...
        table_metadata_uri="s3://df-prod-lakehouse/warehouse/bronze/events_recommendation_tracking/metadata/00001.metadata.json",
...
def recommendation_snapshot_metadata_entry(
    *,
    ...
    return {
        ...
        "metadata_uri": f"s3://df-prod-lakehouse/warehouse/{layer}/{table_name}/metadata/00001.metadata.json",
        "metadata_hash": "sha256:2222222222222222222222222222222222222222222222222222222222222222",
        "manifest_list_uri": f"s3://df-prod-lakehouse/warehouse/{layer}/{table_name}/metadata/snap-00001.avro",
