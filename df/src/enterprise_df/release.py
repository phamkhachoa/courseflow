...
from enterprise_df.access_grants import validate_access_grant_registry
...
from enterprise_df.attestations import validate_evidence_trust_key_registry, verify_external_evidence_attestation
from enterprise_df.catalog import hash_file, load_json
from enterprise_df.contracts import load_yaml, validate_contract_tree
from enterprise_df.domains import validate_domain_registry
from enterprise_df.ingestion import read_jsonl
from enterprise_df.products import validate_product_onboarding_tree
from enterprise_df.quality_profiles import (
    evaluate_quality_profile,
    hash_quality_profile_registry,
    validate_quality_profile_registry,
)
from enterprise_df.retention import validate_retention_policy_registry
from enterprise_df.release_profiles import (
...
def _pipeline_catalog_lineage_gate(
    ...
    *,
    ...
    use_case_output_lineage = any(
        ...
        and edge.get("source") == f"urn:enterprise-df:use-case:{use_case_id}"
        and edge.get("target") == f"urn:enterprise-df:data-product:{primary_output}"
        for edge in edges
    )
    ...
    missing_transform_edges = [
        ...
        if not any(
            ...
            and catalog_edge.get("source") == f"urn:enterprise-df:data-product:{edge['source']}"
            and catalog_edge.get("target") == f"urn:enterprise-df:data-product:{edge['target']}"
    ...
    missing_topic_edges = [
        ...
        if not any(
            ...
            and catalog_edge.get("source") == f"urn:enterprise-df:topic:{edge['source']}"
            and catalog_edge.get("target") == f"urn:enterprise-df:data-product:{edge['target']}"
            for catalog_edge in edges
        )
    ]
...
def pipeline_run_urn(pipeline_manifest: dict[str, Any]) -> str | None:
    ...
    if isinstance(pipeline, str) and isinstance(snapshot_id, str):
        return f"urn:enterprise-df:run:{pipeline}:{snapshot_id}"
    return None
