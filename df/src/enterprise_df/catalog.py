...
from enterprise_df.change_requests import change_request_catalog_entries
...
def data_product_entry(path: Path, contract: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "contract_urn": f"urn:enterprise-df:data-product:{name}:v{version}",
        "name": name,
...
def topic_entry(path: Path, contract: dict[str, Any]) -> dict[str, Any]:
    ...
    version = contract.get("contractVersion")
    return {
        "urn": f"urn:enterprise-df:topic:{name}",
        "contract_urn": f"urn:enterprise-df:topic:{name}:v{version}",
        "name": name,
        "contract_version": version,
        "contract_path": path.as_posix(),
        "contract_hash": hash_file(path),
...
def product_entry(path: Path, onboarding: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "urn": f"urn:enterprise-df:product:{product.get('code')}",
...
def use_case_entry(path: Path, use_case: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "urn": f"urn:enterprise-df:use-case:{use_case_id}",
...
def quality_profile_entry(path: Path, profile: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "urn": f"urn:enterprise-df:quality-profile:{profile_id}",
        "registry_path": path.as_posix(),
        "registry_hash": hash_file(path),
        "id": profile_id,
...
def access_persona_entry(
    ...
    return {
        "urn": f"urn:enterprise-df:access-persona:{persona_id}",
        "registry_path": path.as_posix(),
...
def consumer_contract_entry(
    ...
    return {
        "urn": f"urn:enterprise-df:consumer-contract:{contract_id}",
        "registry_path": path.as_posix(),
        "registry_hash": registry_hash,
        "registry_scope": registry_scope,
        "id": contract_id,
...
def access_grant_entry(
    ...
    return {
        "urn": f"urn:enterprise-df:access-grant:{grant_id}",
        "registry_path": path.as_posix(),
        "registry_hash": registry_hash,
        "registry_scope": registry_scope,
        "id": grant_id,
...
def access_policy_entry(
    ...
    return {
        "urn": f"urn:enterprise-df:access-policy:{policy_id}",
        "registry_path": path.as_posix(),
        "registry_hash": registry_hash,
        "registry_scope": registry_scope,
        "id": policy_id,
...
def retention_policy_entry(
    ...
    policy_id = str(policy.get("id"))
    return {
        "urn": f"urn:enterprise-df:retention-policy:{policy_id}",
        "registry_path": path.as_posix(),
        "registry_hash": registry_hash,
        "registry_scope": registry_scope,
        "id": policy_id,
...
def release_profile_entry(path: Path, profile: dict[str, Any]) -> dict[str, Any]:
    profile_id = str(profile.get("id"))
    return {
        "urn": f"urn:enterprise-df:release-evidence-profile:{profile_id}",
        "registry_path": path.as_posix(),
        "registry_hash": hash_file(path),
        "id": profile_id,
        "name": profile.get("name"),
...
def run_evidence_entry(path: Path) -> dict[str, Any]:
    ...
    return {
        "urn": f"urn:enterprise-df:run:{manifest.get('pipeline')}:{manifest.get('snapshot_id') or manifest.get('ingest_run_id')}",
            ...
            edges.append(
                {
                    ...
                    "source": topic["urn"],
                    "target": f"urn:enterprise-df:data-product:{bronze_target}",
                    "product": topic.get("product"),
                    "domain": topic.get("domain"),
                }
            )
                ...
                edges.append(
                    {
                        ...
                        "source": f"urn:enterprise-df:data-product:{upstream_name}",
                        "target": target,
                ...
                edges.append(
                    {
                        ...
                        "source": source,
                        "target": f"urn:enterprise-df:data-product:{data_product_name}",
                        "domain": use_case.get("domain"),
                        "priority": use_case.get("priority"),
            ...
            edges.append(
                {
                    ...
                    "source": f"urn:enterprise-df:data-product:{source_layer}",
                    "target": f"urn:enterprise-df:data-product:{target_layer}",
                    "run": run["urn"],
            ...
            edges.append(
                {
                    ...
                    "source": f"urn:enterprise-df:topic:{run['topic']}",
                    "target": f"urn:enterprise-df:data-product:{run['bronze_target']}",
                    "run": run["urn"],
                    "product": run.get("product"),
                }
            )
