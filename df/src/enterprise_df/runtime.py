...
from enterprise_df.environments import REQUIRED_ENVIRONMENTS, REQUIRED_P0_SERVICES
...
VALID_PROFILE_STATES = {"runnable_skeleton", "skeleton_not_applied", "planned", "applied"}
...
def validate_iac_registry(
    ...
    validate_modules(root, path, registry.get("modules"), topology_context.get("service_ids", set()), profiles, result)
...
def write_runtime_evidence_pack(
    ...
    *,
    ...
    manifest = {
        ...
        "profile_id": profile_id,
        "source_kind": source_kind,
        "passed": environment == "local" or source_kind != "synthetic_fixture",
        "blockers": (
            [
                {
...
def write_runtime_iac_evidence_pack(
    ...
    *,
    ...
    base = common_evidence_payload(
        ...
        artifact_base_uri=artifact_base_uri,
        change_request_id=change_request_id,
        command="enterprise-df runtime-evidence-normalize-iac",
    )
...
def common_evidence_payload(
    *,
    ...
    command: str = "enterprise-df runtime-evidence-pack",
) -> dict[str, Any]:
