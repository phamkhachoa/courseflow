...
def catalog_metrics(catalog_bundle: dict[str, Any], *, environment: str) -> list[Metric]:
    ...
    for (product, domain, layer), count in sorted(data_product_counts.items()):
        metrics.append(
            Metric(
                name="enterprise_df_catalog_data_products_total",
                value=count,
    ...
    for (product, domain), count in sorted(topic_counts.items()):
        metrics.append(
            Metric(
                name="enterprise_df_catalog_topics_total",
                value=count,
    ...
    for (domain, priority, status), count in sorted(use_case_counts.items()):
        metrics.append(
            Metric(
                name="enterprise_df_catalog_use_cases_total",
                value=count,
...
def run_metrics(
    ...
    *,
    ...
    for run in catalog_bundle.get("run_evidence", []):
        ...
        metrics.append(
            Metric(
                name="enterprise_df_run_quality_status",
        ...
        if isinstance(row_count, int | float):
            metrics.append(
                Metric(
                    name="enterprise_df_run_rows_total",
                    value=row_count,
                    labels=labels,
...
def release_metrics(
    ...
    *,
    ...
    for release in releases:
        ...
        metrics.append(
            Metric(
                name="enterprise_df_release_passed_status",
        ...
        for gate in release.get("gates", []):
            ...
            metrics.append(
                Metric(
                    name="enterprise_df_release_gate_status",
        ...
        if isinstance(freshness, dict):
            for data_product_name, report in freshness.items():
                ...
                metrics.append(
                    Metric(
                        name="enterprise_df_data_product_freshness_seconds",
                        value=age_seconds,
                        labels={
                            "environment": environment,
        ...
        if isinstance(quality, dict):
            ...
            if isinstance(quarantine_rows, int | float):
                metrics.append(
                    Metric(
                        name="enterprise_df_quarantine_rows",
        ...
        if isinstance(details, dict) and isinstance(details.get("max_lag_seconds"), int | float):
            ...
            metrics.append(
                Metric(
                    name="enterprise_df_ingestion_lag_seconds",
...
def cost_metrics(
    ...
    *,
    ...
    return [
        Metric(
            name="enterprise_df_cost_attribution_rows_total",
...
def validate_observability_snapshot(snapshot: dict[str, Any]) -> ValidationResult:
    ...
    if snapshot.get("artifact_type") != "enterprise_df_observability_snapshot.v1":
        result.error(Path("observability"), "artifact_type must be enterprise_df_observability_snapshot.v1")
