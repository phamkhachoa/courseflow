...
`df/platform/observability/production-slo-release-gates.md`.
...
- `enterprise_df_ingestion_lag_seconds` exceeds the 15 minute SLO.
- `enterprise_df_partition_stalled` is `1`.
...
2. Re-run `enterprise-df schema-registry-check` locally and the external registry workflow in CI.
...
2. Re-run `enterprise-df access-policy-check` or the production policy workflow.
3. Attach the new report URI and hash to the release evidence.
...
- `enterprise_df_gold_publish_evidence_status` is `0`.
