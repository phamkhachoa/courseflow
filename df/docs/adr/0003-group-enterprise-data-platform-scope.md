...
The initial `df/` work started from CourseFlow LMS because it is the first product with concrete
source systems, recommendation ML needs, incentive/finance data and operational analytics pressure.
The group now needs `df/` to support many enterprise products, not only LMS.
...
`df/` is the group-wide enterprise data platform. CourseFlow LMS is registered as the first product
...
- Tooling is renamed from `courseflow_df` to `enterprise_df`.
