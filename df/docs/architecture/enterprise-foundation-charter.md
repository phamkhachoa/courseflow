# Enterprise Data Foundation Charter
...
`df/` is the group-wide enterprise data foundation for the corporation. It is not an LMS data mart,
...
- Shared standards under `platform/`, `catalog/`, `governance/`, `templates/` and
  `src/enterprise_df/` must use enterprise-neutral vocabulary unless they are explicitly marked as a
...
enterprise data foundation brings contracts, landing, lakehouse storage, transformations, quality,
catalog, security, release governance and operational evidence.

Any change that makes `df/` hard to reuse for the next non-LMS product must be treated as an
