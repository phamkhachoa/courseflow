...
This runbook defines the local P0 slice used to prove the enterprise DF operating pattern:
...
PYTHONPATH=src python -m enterprise_df.cli run-use-case \
  ...
  --output-dir /tmp/enterprise-df/ml-feature-governance \
...
PYTHONPATH=src python -m enterprise_df.cli run-use-case \
  ...
  --output-dir /tmp/enterprise-df/finance-benefit-reconciliation \
...
```bash
PYTHONPATH=src python -m enterprise_df.cli run-recommendation-slice \
  --root . \
  --input samples/recommendation/tracking.jsonl \
  --output-dir /tmp/enterprise-df/recommendation-slice \
