...
/**
 * Holds and validates the shared JWT signing secret for identity-service.
 *
 * <p>The secret is validated <em>fail-fast</em> at startup: the application refuses to boot when the
 * secret is missing, blank, shorter than {@value #MIN_SECRET_BYTES} bytes, or still set to the
 * well-known local-development placeholder. This prevents shipping a build that signs/verifies
 * tokens with a guessable key. The same secret is shared with the gateway (HS256).
 */
@Component
public class JwtProperties {
    ...
    public JwtProperties(
            @Value("${courseflow.security.jwt.secret:}") String secret,
            @Value("${courseflow.security.jwt.access-token-ttl-seconds:900}") long accessTokenTtlSeconds,
            @Value("${courseflow.security.jwt.issuer:courseflow-identity}") String issuer,
            @Value("${courseflow.security.jwt.clock-skew-seconds:60}") long clockSkewSeconds) {
    ...
    static String validate(String secret) {
        if (secret == null || secret.isBlank()) {
            throw new IllegalStateException(
                    "COURSEFLOW_JWT_SECRET is not set. Identity-service refuses to start without a JWT signing secret. "
