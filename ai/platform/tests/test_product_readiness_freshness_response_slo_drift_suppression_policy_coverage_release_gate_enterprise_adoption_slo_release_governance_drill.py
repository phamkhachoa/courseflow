...
build_drill = (
    drill.build_product_readiness_freshness_response_slo_drift_suppression_policy_coverage_release_gate_enterprise_adoption_slo_release_governance_drill_report
)
build_drill_from_governance = (
    drill.build_product_readiness_freshness_response_slo_drift_suppression_policy_coverage_release_gate_enterprise_adoption_slo_release_governance_drill_report_from_governance
)
build_snapshot = (
    drill.build_product_readiness_freshness_response_slo_drift_suppression_policy_coverage_release_gate_enterprise_adoption_slo_release_governance_drill_snapshot
)
build_release_governance = (
    governance.build_product_readiness_freshness_response_slo_drift_suppression_policy_coverage_release_gate_enterprise_adoption_slo_release_governance_report
)
...
def test_enterprise_adoption_slo_release_governance_drill_passes() -> None:
    report = build_drill(ai_root(), generated_at="2026-06-17")

    assert report.drill_status == "passed"
    assert (
        report.release_governance_status
...
def test_enterprise_adoption_slo_release_governance_drill_blocks_gap() -> None:
    ...
    assert report.next_actions == (
        "exercise_enterprise_release_gate_pattern_adoption_slo_release_"
        "governance_drill",
    )


def test_enterprise_adoption_slo_release_governance_drill_detects_gap() -> None:
    release_governance = build_release_governance(
        ai_root(),
        generated_at="2026-06-17",
    )
    broken_gate = replace(
        ...
        attached=False,
        validation_errors=("release governance attachment missing",),
    )
    report = build_drill_from_governance(
        replace(
            release_governance,
    ...
    assert (
        report.scenarios[0].gate_id
        == "enterprise-adoption-slo-published-release-governance-gate"
    )
    assert report.scenarios[0].passed is False
    assert (
        report.scenarios[0].observed_outcome
        == "enterprise_adoption_slo_release_governance_gap_detected"
    )


def test_enterprise_adoption_slo_governance_drill_snapshot_is_tenant_safe() -> None:
...
def test_enterprise_adoption_slo_governance_drill_raw_id_detector() -> None:
    report = build_drill(ai_root(), generated_at="2026-06-17")
    ...
    assert (
        drill.count_raw_identifier_markers(
            (leaked_scenario, *report.scenarios[1:])
        )
        == 1
    )


def test_enterprise_adoption_slo_governance_drill_snapshot_matches() -> None:
    root = ai_root()
    report_name = (
        "product-readiness-freshness-response-slo-drift-suppression-policy-"
        "coverage-release-gate-enterprise-adoption-slo-release-governance-drill-v1.yaml"
    )
