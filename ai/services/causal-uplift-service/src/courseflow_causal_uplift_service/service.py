...
from courseflow_ai_platform.causal_uplift_service import (
    ...
    CAUSAL_UPLIFT_OPS_SCOPE,
    CausalUpliftAccessPolicy,
...
CAUSAL_UPLIFT_SERVICE_ID = "causal-uplift-service"


@dataclass(frozen=True, slots=True)
class CausalUpliftServiceResponse:
    status_code: int
    body: dict[str, Any]
...
SERVICE_ROUTES = (
    ServiceRoute(
        method="POST",
        path="/v1/causal-uplift/evaluate",
        scope=CAUSAL_UPLIFT_EVALUATE_SCOPE,
        purpose="aggregate_experiment_uplift_evaluation",
    ),
    ServiceRoute(
        method="GET",
        path="/v1/causal-uplift/health",
        scope=CAUSAL_UPLIFT_OPS_SCOPE,
        purpose="causal_uplift_health",
    ),
    ServiceRoute(
        method="GET",
        path="/v1/causal-uplift/metrics",
        scope=CAUSAL_UPLIFT_OPS_SCOPE,
        purpose="causal_uplift_metrics",
    ),
)


@dataclass(frozen=True, slots=True)
class CausalUpliftServiceConfig:
    ai_root: Path
...
class CausalUpliftService:
    """Policy-enforced service boundary around causal uplift runtime."""

    def __init__(self, config: CausalUpliftServiceConfig) -> None:
        self.config = config
        self.access_policy: CausalUpliftAccessPolicy | None = (
            load_causal_uplift_access_policy(config.ai_root)
            if config.auth_enabled
            else None
        )
        self.runtime = CausalUpliftRuntime(config.ai_root)
    ...
    def handle_request(
        ...
        *,
        ...
        if self.config.auth_enabled and resolved_principal is None:
            if principal_id is None:
                return CausalUpliftServiceResponse(
                    ...
                    body={
                        ...
                        "errorMessage": "causal uplift principal is required",
                    },
                )
            try:
            ...
            except CausalUpliftServiceError as exc:
                return error_response(403, "principal_policy_forbidden", str(exc))

        try:
            if route == ("POST", "/v1/causal-uplift/evaluate"):
                return self.evaluate(body, resolved_principal)
            if route == ("GET", "/v1/causal-uplift/health"):
                return self.health()
            if route == ("GET", "/v1/causal-uplift/metrics"):
                return self.metrics()
        except CausalUpliftPrivacyError as exc:
            return error_response(403, "privacy_control_violation", str(exc))
        except CausalUpliftServiceError as exc:
            return error_response(400, "bad_request", str(exc))
        except Exception as exc:
            return error_response(500, "causal_uplift_service_failed", str(exc))
    ...
    def health(self) -> CausalUpliftServiceResponse:
        return CausalUpliftServiceResponse(status_code=200, body=self.runtime.health())

    def metrics(self) -> CausalUpliftServiceResponse:
        return CausalUpliftServiceResponse(
            status_code=200,
    ...
    def resolve_principal(
        ...
        *,
        ...
        if self.access_policy is None:
            raise CausalUpliftServiceError("causal uplift access policy is disabled")
        return self.access_policy.resolve_principal(principal_id, requested_scopes)


def build_service_manifest() -> dict[str, Any]:
    return {
        "serviceId": CAUSAL_UPLIFT_SERVICE_ID,
        "owner": "ai-platform",
        "status": "service_package_ready",
        "runtime": {
            "entrypoint": "courseflow_causal_uplift_service.cli:main",
            "language": "python",
        },
        "security": {
        ...
        "observability": {
            "metrics": "in_memory_evaluation_error_identifier_guardrail_hitl_counters",
            "tenantSafety": "cross_tenant_causal_uplift_forbidden",
        },
