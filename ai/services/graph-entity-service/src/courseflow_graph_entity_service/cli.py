...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="CourseFlow AI Platform graph entity service."
    )
    ...
    analyze.add_argument("--body-json")
    analyze.add_argument("--body-file", type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = GraphEntityService(
        GraphEntityServiceConfig.from_paths(
    ...
    if args.command == "serve":
        from courseflow_graph_entity_service.http_server import serve_http
...
def load_body(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.body_file:
        ...
        raise SystemExit("analyze requires --body-json or --body-file.")
    payload = json.loads(raw)
    if not isinstance(payload, dict):
        raise SystemExit("analyze body must be a JSON object.")
    return payload
