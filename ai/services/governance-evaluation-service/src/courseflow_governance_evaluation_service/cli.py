...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="CourseFlow AI Platform governance evaluation service."
    )
    ...
    assess.add_argument("--body-json")
    assess.add_argument("--body-file", type=Path)

    serve = subparsers.add_parser("serve")
    serve.add_argument("--host", default="127.0.0.1")
    serve.add_argument("--port", type=int, default=8103)
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = GovernanceEvaluationService(
        GovernanceEvaluationServiceConfig.from_paths(
    ...
    if args.command == "serve":
        from courseflow_governance_evaluation_service.http_server import serve_http
    ...
    if args.command == "health":
        return print_response(
            service.handle_request(
                "GET",
                "/v1/governance-evaluation/health",
                principal_id=args.principal_id,
                requested_scopes=requested_scopes,
            )
        )
...
def load_body(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.body_file:
        ...
        raise SystemExit("assess requires --body-json or --body-file.")
    payload = json.loads(raw)
    if not isinstance(payload, dict):
        raise SystemExit("assess body must be a JSON object.")
    return payload
