...
from courseflow_ai_platform.payment_fraud_service import (
    PAYMENT_FRAUD_OPS_SCOPE,
    PAYMENT_FRAUD_SCORE_SCOPE,
    PaymentFraudAccessPolicy,
...
PAYMENT_FRAUD_SERVICE_ID = "payment-fraud-service"


@dataclass(frozen=True, slots=True)
class PaymentFraudServiceResponse:
    status_code: int
    body: dict[str, Any]
...
@dataclass(frozen=True, slots=True)
class PaymentFraudServiceConfig:
    ai_root: Path
...
class PaymentFraudService:
    """Policy-enforced service boundary around payment fraud runtime."""

    def __init__(self, config: PaymentFraudServiceConfig) -> None:
        self.config = config
        self.access_policy: PaymentFraudAccessPolicy | None = (
            load_payment_fraud_access_policy(config.ai_root)
            if config.auth_enabled
            else None
        )
        self.runtime = PaymentFraudRuntime(config.ai_root)
    ...
    def handle_request(
        ...
        *,
        ...
        if self.config.auth_enabled and resolved_principal is None:
            if principal_id is None:
                return PaymentFraudServiceResponse(
                    ...
                    body={
                        ...
                        "errorMessage": "payment fraud principal is required",
                    },
                )
            try:
            ...
            except PaymentFraudServiceError as exc:
                return error_response(403, "principal_policy_forbidden", str(exc))

        try:
            if route == ("POST", "/v1/payment-fraud/score"):
                return self.score(body, resolved_principal)
            if route == ("GET", "/v1/payment-fraud/health"):
                return self.health()
            if route == ("GET", "/v1/payment-fraud/metrics"):
                return self.metrics()
        except PaymentFraudPrivacyError as exc:
            return error_response(403, "privacy_control_violation", str(exc))
        except PaymentFraudServiceError as exc:
            return error_response(400, "bad_request", str(exc))
        except Exception as exc:
            return error_response(500, "payment_fraud_service_failed", str(exc))
    ...
    def health(self) -> PaymentFraudServiceResponse:
        return PaymentFraudServiceResponse(status_code=200, body=self.runtime.health())

    def metrics(self) -> PaymentFraudServiceResponse:
        return PaymentFraudServiceResponse(
            status_code=200,
...
def build_service_manifest() -> dict[str, Any]:
    return {
        ...
        "observability": {
            ...
            "tenantSafety": "cross_tenant_payment_fraud_forbidden",
        },
