...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="CourseFlow AI Platform prompt gateway service."
    )
    ...
    evaluate.add_argument("--body-json")
    evaluate.add_argument("--body-file", type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = PromptGatewayService(
        PromptGatewayServiceConfig.from_paths(
    ...
    if args.command == "serve":
        from courseflow_prompt_gateway_service.http_server import serve_http
...
def load_body(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.body_file:
        ...
        raise SystemExit("evaluate requires --body-json or --body-file.")
    payload = json.loads(raw)
    if not isinstance(payload, dict):
        raise SystemExit("evaluate body must be a JSON object.")
    return payload
