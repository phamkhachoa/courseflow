...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CourseFlow AI Platform sequence risk service.")
    ...
    score.add_argument("--body-json")
    score.add_argument("--body-file", type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = SequenceRiskService(
        SequenceRiskServiceConfig.from_paths(
    ...
    if args.command == "serve":
        from courseflow_sequence_risk_service.http_server import serve_http
...
def load_body(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.body_file:
        ...
        raise SystemExit("score requires --body-json or --body-file.")
    payload = json.loads(raw)
    if not isinstance(payload, dict):
        raise SystemExit("score body must be a JSON object.")
    return payload
