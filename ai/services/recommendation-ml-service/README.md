...
Canonical location: `ai/services/recommendation-ml-service`.
...
This service is now the first runtime service under the wider `ai/` platform. Add new packages under
`training/` only when they serve the recommendation domain, such as content-similarity fallback,
personalized ranking, two-tower retrieval, SASRec sequence recommendation or re-ranking for related
courses.

New AI domains should start as platform artifacts first:

- `ai/use-cases/<use-case>/`
- `ai/contracts/features/*.yaml`
- `ai/contracts/models/*.yaml`
- `ai/platform/evaluation/quality-gates/*.yaml`
- `ai/platform/governance/policies/*.yaml`

Promote them to sibling services under `ai/services/` only when they need independent storage,
security scopes, runtime SLA or service ownership. Examples: semantic search, RAG tutor,
auto-grading, learner-risk prediction, knowledge tracing and adaptive learning path.
