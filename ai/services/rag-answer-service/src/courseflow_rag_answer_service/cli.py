...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="CourseFlow AI Platform RAG answer service."
    )
    ...
    answer.add_argument("--body-json")
    answer.add_argument("--body-file", type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    service = RagAnswerService(
        RagAnswerServiceConfig.from_paths(
    ...
    if args.command == "serve":
        from courseflow_rag_answer_service.http_server import serve_http
...
def load_body(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.body_file:
        ...
        raise SystemExit("answer requires --body-json or --body-file.")
    payload = json.loads(raw)
    if not isinstance(payload, dict):
        raise SystemExit("answer body must be a JSON object.")
    return payload
